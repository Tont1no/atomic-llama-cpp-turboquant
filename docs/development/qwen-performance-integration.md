...
rollback layout. A pure DSpark chain can use dynamic recurrent depth because
its fixed, SPS-planned, shadow, and recorder batches all carry exact packed
per-row depth metadata, but compact residency is fail-closed off by default.
Enable it only after growth headroom is qualified with
`--spec-draft-dynamic-rs` (or `LLAMA_ARG_SPEC_DRAFT_DYNAMIC_RS=1`). Keep the
default or pass `--no-spec-draft-dynamic-rs` to allocate the fixed configured
rollback depth at startup and avoid transient old-plus-new resize overlap.
