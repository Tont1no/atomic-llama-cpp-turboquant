...
ALLOWED_ACTIVE_SLOTS = set(range(1, 9))
...
ATOMIC_RECORDER_READ_RETRY_SECONDS = 3.0
...
RECORDER_RETAINED_METRIC = "llamacpp:spec_decode_sps_record_retained_samples_total"
RECORDER_WARMUP_METRIC = "llamacpp:spec_decode_sps_record_skipped_warmup_total"
RECORDER_READY_METRIC = "llamacpp:spec_decode_sps_record_coordinates_ready"
RECORDER_TOTAL_METRIC = "llamacpp:spec_decode_sps_record_coordinates_total"
RECORDER_FATAL_METRICS = {
    "llamacpp:spec_decode_sps_record_skipped_retry_total",
    "llamacpp:spec_decode_sps_record_write_errors_total",
}
RECORDER_FINAL_ZERO_METRICS = RECORDER_FATAL_METRICS | {
    "llamacpp:spec_decode_sps_record_skipped_mixed_total",
    "llamacpp:spec_decode_sps_record_skipped_partial_total",
}
RECORDER_DIAGNOSTIC_METRICS = {
    "llamacpp:spec_decode_sps_record_skipped_capture_total",
    "llamacpp:spec_decode_sps_record_skipped_other_total",
}
RECORDER_REQUIRED_METRICS = {
    RECORDER_RETAINED_METRIC,
    RECORDER_WARMUP_METRIC,
    RECORDER_READY_METRIC,
    RECORDER_TOTAL_METRIC,
} | RECORDER_FINAL_ZERO_METRICS | RECORDER_DIAGNOSTIC_METRICS
ROOT_KEYS = {
    "schema",
    "seed",
    "schedule_strategy",
...
def _windows_shared_atomic_recorder_bytes(path: Path) -> bytes:
    """Read one stable file generation without blocking an atomic replacement."""
    if os.name != "nt":
        raise ContractError("Windows shared recorder reads require Windows")
    ...
    # Atomic replacement on Windows needs every live reader to share deletion,
    # in addition to ordinary read/write sharing.  The open handle continues to
    # name one immutable generation even if the directory entry is replaced.
    handle = create_file(
        str(path),
        ...
        None,
        3,  # OPEN_EXISTING
        0x00000080 | 0x08000000,  # FILE_ATTRIBUTE_NORMAL | SEQUENTIAL_SCAN
        None,
    )
    ...
    try:
        ...
        if not get_file_size(handle, ctypes.byref(size)):
            raise ctypes.WinError(ctypes.get_last_error())
        if size.value <= 0 or size.value > MAX_PLAN_BYTES:
            raise ContractError(f"plan size must be in [1, {MAX_PLAN_BYTES}] bytes")
        ...
        while offset < size.value:
            ...
            request = min(size.value - offset, 1024 * 1024)
            if not read_file(
                handle,
                ...
                raise ctypes.WinError(ctypes.get_last_error())
            if read.value <= 0 or read.value > request:
                raise ContractError(f"atomic recorder file changed while reading: {path}")
            offset += read.value
        return bytes(buffer.raw)
    finally:
        # Never retain a generation across the caller's JSON/schema checks.
        close_handle(handle)


def _load_windows_shared_atomic_recorder_json(path: Path) -> Any:
    try:
        payload = _windows_shared_atomic_recorder_bytes(path)
    except ContractError:
        raise
    except OSError as error:
        raise ContractError(f"cannot read strict UTF-8 JSON plan: {path}") from error
...
def _is_transient_atomic_recorder_error(error: OSError) -> bool:
    if os.name != "nt":
        # Preserve the existing non-Windows retry policy.  The special shared
        # reader is Windows-only and does not alter other platforms' I/O path.
        return True
    return getattr(error, "winerror", None) in {
        2,    # ERROR_FILE_NOT_FOUND during initial publication
        3,    # ERROR_PATH_NOT_FOUND during initial publication
        5,    # ERROR_ACCESS_DENIED while a generation is being retired
...
def load_atomic_recorder_json(
    path: Path,
    *,
    deadline: float,
    guard_check: Callable[[], Any] | None = None,
) -> Any:
    if not math.isfinite(deadline):
        raise ContractError("atomic recorder read deadline must be finite")
    while True:
        try:
            if os.name == "nt":
                return _load_windows_shared_atomic_recorder_json(path)
            return load_json(path)
        except ContractError as error:
            # Recorder JSON is atomically replaced, so malformed UTF-8/JSON is
            # never a valid intermediate state. Retry only filesystem access
            # failures such as a transient Windows sharing violation.
            if (
                not isinstance(error.__cause__, OSError)
                or not _is_transient_atomic_recorder_error(error.__cause__)
            ):
                raise
...
def build_plan(
    *,
    ...
    if set(active_slots) - ALLOWED_ACTIVE_SLOTS:
        raise ContractError("active_slots may only contain integers from 1 through 8")
    if prefix_caps[0] != 0:
...
def gpu_identity(repo_root: Path, expected_uuid: str) -> dict[str, str]:
    ...
    runtime_match = re.search(
        r"CUDA(?:\s+UMD)?\s+Version:\s*([0-9]+(?:\.[0-9]+)*)", banner
    )
    if runtime_match is None:
        raise ContractError("nvidia-smi did not report a CUDA driver runtime version")
    return {
        "uuid": parts[0],
        "name": parts[1],
...
def windows_job_active_process_count(job_name: str) -> int:
    if os.name != "nt":
        raise ContractError("exclusive GPU guard requires Windows Job Object accounting")
    ...
    try:
        if not query_job(
            ...
            raise ContractError("exclusive GPU Job Object accounting query failed")
    finally:
        close_handle(handle)
...
def stop_process_tree(
    ...
    *,
    ...
    expected_job_processes: int | None = None,
) -> None:
    if (
        ...
        raise ContractError("server process cleanup timeout must be positive and finite")
    ...
    if process.poll() is None:
        ...
        if wait_timeout <= 0:
            raise ContractError(f"server process did not exit: pid={process.pid}")
        try:
            process.wait(timeout=wait_timeout)
        except subprocess.TimeoutExpired as error:
            raise ContractError(f"server process did not exit: pid={process.pid}") from error

    if os.name == "nt" and job_name is not None:
        if expected_job_processes is None or expected_job_processes < 1:
            raise ContractError("server cleanup requires a valid Job Object baseline")
        while True:
            active_processes = windows_job_active_process_count(job_name)
            if active_processes == expected_job_processes:
                break
            if active_processes < expected_job_processes:
                raise ContractError("exclusive GPU Job Object process count fell below its baseline")
            wait_timeout = deadline - time.monotonic()
            if wait_timeout <= 0:
                raise ContractError(
                    "server descendants remained in the exclusive GPU Job Object after cleanup"
                )
            time.sleep(min(0.05, wait_timeout))
...
def validate_sidecar(
    ...
    *,
    ...
    if require_ready and root["skipped_ineligible_total"] != 0:
        raise ContractError("completed sidecar contains ineligible target decodes")
    return root
...
def server_arguments(
    ...
    values.append("--spec-draft-dynamic-rs" if args.dynamic_rs else "--no-spec-draft-dynamic-rs")
...
def validate_recorder_metrics(
    ...
    *,
    ...
    missing = RECORDER_REQUIRED_METRICS - metrics.keys()
    if missing:
        raise ContractError(f"recorder metrics are missing: {sorted(missing)}")

    values: dict[str, int] = {}
    for name in RECORDER_REQUIRED_METRICS:
        value = metrics[name]
        if not math.isfinite(value) or value < 0 or not float(value).is_integer():
            raise ContractError(f"recorder metric is not a non-negative integer: {name}")
        values[name] = int(value)
        if previous is not None:
            old = previous.get(name)
            if old is None or value < old:
                raise ContractError(f"recorder metric is missing or decreased: {name}")

    nonzero_failures = [name for name in sorted(RECORDER_FATAL_METRICS) if values[name] != 0]
    if nonzero_failures:
        raise ContractError(f"recorder reported a retry/write-error counter: {nonzero_failures}")
    if require_complete:
        final_failures = [
            name for name in sorted(RECORDER_FINAL_ZERO_METRICS) if values[name] != 0
        ]
        if final_failures:
            raise ContractError(f"recorder reported an invalid generation counter: {final_failures}")
    retained = values[RECORDER_RETAINED_METRIC]
    warmup = values[RECORDER_WARMUP_METRIC]
    ready = values[RECORDER_READY_METRIC]
    total = values[RECORDER_TOTAL_METRIC]
    if retained > samples_per_cell:
        raise ContractError("recorder retained-sample counter exceeded the configured target")
    if total != 1 or ready not in {0, 1}:
        raise ContractError("recorder coordinate gauges do not match the one-cell arm")
    complete = retained == samples_per_cell and warmup >= warmup_shape_runs and ready == 1
    if ready == 1 and not complete:
        raise ContractError("recorder reported ready before all expected samples")
    if require_complete and not complete:
        raise ContractError("recorder metrics did not prove complete sample retention")
    return complete, (retained, ready)
...
def run_wave(
    *,
    ...
    try:
        ...
        recorder_complete, last_recorder_progress = validate_recorder_metrics(
            initial_metrics,
            samples_per_cell=args.samples_per_cell,
            warmup_shape_runs=args.warmup_shape_runs,
            require_complete=False,
        ...
        while any(worker.is_alive() for worker in workers):
            ...
            now = time.monotonic()
            if now - last_metrics_poll >= max(0.05, args.poll_interval_s):
                progress_remaining = max(
                    0.001,
                    args.progress_timeout_s - (now - last_progress),
                )
                current_metrics = parse_metrics(
                    http_text(
                        base_url,
                        "/metrics",
                        min(
                ...
                recorder_complete, recorder_progress = validate_recorder_metrics(
                    current_metrics,
                    samples_per_cell=args.samples_per_cell,
                    warmup_shape_runs=args.warmup_shape_runs,
                ...
                if recorder_progress != last_recorder_progress:
                    last_progress = now
                last_recorder_progress = recorder_progress
                last_metrics = current_metrics
                last_metrics_poll = now
            if not recorder_complete and time.monotonic() - last_progress > args.progress_timeout_s:
        ...
        final_metrics = parse_metrics(
            http_text(
                base_url,
                "/metrics",
                min(args.http_timeout_s, remaining(arm_deadline, "arm")),
            )
        )
        validate_recorder_metrics(
            final_metrics,
...
def run_arm(
    *,
    ...
    guard_job_name = os.environ.get(GUARD_JOB_NAME_ENV) if os.name == "nt" else None
    guard_job_processes = None
    if guard_job_name is not None:
        guard_job_processes = windows_job_active_process_count(guard_job_name)
        if guard_job_processes != 1:
            raise ContractError(
                "exclusive GPU Job Object must contain only the runner before a server arm"
            )
    ...
    try:
        ...
        metrics_before_values = parse_metrics(metrics_before)
        validate_recorder_metrics(
        ...
        client = run_wave(
            ...
            abort_server=lambda: stop_process_tree(
                process,
                args.stop_timeout_s,
                job_name=guard_job_name,
                expected_job_processes=guard_job_processes,
            ),
            initial_metrics=metrics_before_values,
            guard_check=lambda: verify_guard_lease(repo_root),
        )
        write_json(paths["client"], client, True)
        recorder_read_deadline = min(
            arm_deadline,
            time.monotonic() + ATOMIC_RECORDER_READ_RETRY_SECONDS,
        )
        recorder_guard_check = lambda: verify_guard_lease(repo_root)
        sidecar = validate_sidecar(
            load_atomic_recorder_json(
                paths["sidecar"],
                deadline=recorder_read_deadline,
                guard_check=recorder_guard_check,
            ),
            source_identity_value=source_identity_value,
            cell=cell,
            max_draft_tokens_per_slot=args.max_draft_tokens_per_slot,
        ...
        arm_profile = validate_arm_profile(
            ...
            cell,
            args.max_draft_tokens_per_slot,
        )
        if float(arm_profile["entries"][0]["cost_us"]) != float(sidecar["coordinates"][0]["p95_us"]):
            raise ContractError("arm profile cost does not match the raw sidecar p95")
        ...
        validate_recorder_metrics(
            ...
            samples_per_cell=args.samples_per_cell,
            warmup_shape_runs=args.warmup_shape_runs,
            previous=metrics_before_values,
            require_complete=True,
        )
        receipt = {
            "schema": ARM_SCHEMA,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--dynamic-rs", action=argparse.BooleanOptionalAction, default=False)
