...
static constexpr int32_t SPEC_ADAPTIVE_ARMS[] = { 0, 1, 2, 3, 7 };
...
int32_t common_speculative_adaptive_n_max(
    ...
    if (active_slots <= 0 || hard_cap <= 0 || params.n_max <= 0 || params.adaptive_load_caps.empty()) {
        return 0;
    }

    if (!std::isfinite(params.adaptive_ema_alpha) || params.adaptive_ema_alpha < 0.0f || params.adaptive_ema_alpha > 1.0f ||
        !std::isfinite(params.adaptive_ema_threshold) || params.adaptive_ema_threshold < 0.0f || params.adaptive_ema_threshold > 1.0f) {
        return 0;
    }

    const size_t load_idx = std::min<size_t>((size_t) active_slots - 1, params.adaptive_load_caps.size() - 1);
    const int32_t load_cap = params.adaptive_load_caps[load_idx];
    if (std::find(std::begin(SPEC_ADAPTIVE_ARMS), std::end(SPEC_ADAPTIVE_ARMS), load_cap) == std::end(SPEC_ADAPTIVE_ARMS)) {
        return 0;
    }

    const int32_t capped = std::min({ hard_cap, params.n_max, load_cap });
    if (capped < params.n_min) {
        return 0;
    }

    int32_t selected = 0;
    for (int32_t arm : SPEC_ADAPTIVE_ARMS) {
        if (arm > capped) {
            break;
        }
        selected = arm;
    ...
    if (params.adaptive_ema_alpha <= 0.0f || selected == 0) {
        return selected;
    }

    // Find the longest arm whose sufficiently-observed positions all meet the
    // prefix-survival threshold. Unknown positions are allowed during warm-up;
    // missing or non-finite state after warm-up fails closed at that position.
    int32_t accepted_arm = 0;
    for (int32_t arm : SPEC_ADAPTIVE_ARMS) {
        if (arm == 0) {
            continue;
        }
        ...
        bool arm_ok = true;
        for (int32_t pos = 0; pos < arm; ++pos) {
            const uint64_t offered = (size_t) pos < offered_per_pos.size() ? offered_per_pos[pos] : 0;
            if (offered < params.adaptive_ema_warmup) {
                continue;
            }
            if ((size_t) pos >= acceptance_ema.size() || !std::isfinite(acceptance_ema[pos]) ||
                    acceptance_ema[pos] < params.adaptive_ema_threshold) {
                arm_ok = false;
                break;
            }
        }
    ...
    return accepted_arm >= params.n_min ? accepted_arm : 0;
}
...
void common_speculative_adaptive_acceptance_update(
        ...
        size_t n_accepted,
        float alpha) {
    if (n_offered == 0 || !std::isfinite(alpha) || alpha <= 0.0f || alpha > 1.0f) {
        return;
    }

    if (acceptance_ema.size() < n_offered) {
        acceptance_ema.resize(n_offered, -1.0f);
    }

    for (size_t pos = 0; pos < n_offered; ++pos) {
        const float sample = pos < n_accepted ? 1.0f : 0.0f;
        float & ema = acceptance_ema[pos];
        ema = ema < 0.0f ? sample : alpha * sample + (1.0f - alpha) * ema;
    }
}
...
struct common_speculative_impl {
    ...
    std::vector<size_t> n_gen_tokens_per_pos; // number of times each draft position was offered.
...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    // Once an adaptive request falls back to n=0, keeping the draft KV in sync
    // would retain most of DFlash's overhead. Disable maintenance for that
    // sequence until begin() starts a new request.
    ...
    bool process(const llama_batch & batch_in) override {
        ...
        for (int32_t i = 0; i < n_tokens; ++i) {
            ...
            if (batch_in.n_seq_id[i] == 1 && batch_in.seq_id[i] &&
                    ...
                    seq_enabled[batch_in.seq_id[i][0]]) {
...
bool common_speculative_uses_dynamic_rs(const common_params_speculative & params) {
    // Keep selection independent of the selected valid SPS mode: every
    // pure DSpark execution path uses packed source-order rows with an exact
    // per-row rollback-depth tag. Mixed speculative chains fail closed to the
    // fixed resident allocation.
    if (common_speculative_sps_conflicts_with_adaptive(params)) {
        return false;
    }
    return common_speculative_is_only_dspark(params.types) ||
           (params.draft.adaptive && common_speculative_is_only_dflash_family(params.types));
}
...
void common_speculative_print_stats(const common_speculative * spec) {
    ...
    for (const auto & impl : spec->impls) {
        ...
        if (impl->n_call_accept > 0) {
            ...
            const size_t n_pos = std::max(impl->n_acc_tokens_per_pos.size(), impl->n_gen_tokens_per_pos.size());
            for (size_t i = 0; i < n_pos; ++i) {
                if (i > 0) {
                    tmp << ", ";
                }
                const size_t offered  = i < impl->n_gen_tokens_per_pos.size() ? impl->n_gen_tokens_per_pos[i] : 0;
                const size_t accepted = i < impl->n_acc_tokens_per_pos.size() ? impl->n_acc_tokens_per_pos[i] : 0;
                tmp << (offered > 0 ? (double) accepted / (double) offered : 0.0);
            }
            std::ostringstream oss;
