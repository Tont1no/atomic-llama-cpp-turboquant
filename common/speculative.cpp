...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            const int32_t n_draft = dp.n_max < 0 ? params.n_max :
                    std::min(params.n_max, std::max(0, dp.n_max));
            if (is_dspark && n_draft < params.n_max) {
            ...
            // the per-sequence limit only while sampling the result. Plain
            // DFlash retains its supported per-sequence ragged block.
            const int32_t n_block_tokens = is_dspark ? params.n_max : n_draft + 1;
