...
struct common_sampler {
    ...
    void set_logits(const llama_sampling_output & output) {
        const float *       sampled_probs  = output.probs;
        const float *       sampled_logits = output.backend_logits ? output.logits : nullptr;
        const llama_token * sampled_ids    = output.candidates;
        const uint32_t n_vocab = output.logits_count;

        if (sampled_probs) {
            const uint32_t sampled_probs_count = output.probs_count;
            cur.resize(sampled_probs_count);
            for (uint32_t i = 0; i < sampled_probs_count; ++i) {
                cur[i] = llama_token_data{sampled_ids[i], sampled_logits[i], sampled_probs[i]};
            }
        } else if (sampled_logits) {
            const uint32_t sampled_logits_count = output.logits_count;
            ...
            for (uint32_t token_id = 0; token_id < n_vocab; token_id++) {
                cur[token_id] = llama_token_data{static_cast<llama_token>(token_id), logits[token_id], 0.0f};
            }
        }
...
llama_token common_sampler_sample(struct common_sampler * gsmpl, struct llama_context * ctx, int idx, bool grammar_first) {
    const auto output = llama_get_sampling_output_ith(ctx, idx);
