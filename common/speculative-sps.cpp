...
common_speculative_sps_profile common_speculative_sps_profile_parse(
    ...
    for (const int32_t context : contexts) {
        for (const int32_t active : slot_counts) {
            for (const int32_t rows_count : row_counts) {
                if (rows_count < active) {
                    continue;
                }
                if (coordinates.count(std::make_tuple(context, active, rows_count)) == 0) {
