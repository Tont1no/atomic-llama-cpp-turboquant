...
static std::vector<llama_device_memory_data> common_get_device_memory_data_impl(
    ...
    llama_context * ctx = llama_init_from_model_with_recurrent_cache_type(model, *cparams, type_s);
    if (ctx == nullptr) {
...
common_device_memory_data_vec common_get_device_memory_data(
    ...
    return common_get_device_memory_data(
            path_model, mparams, cparams, devs, hp_ngl, hp_n_ctx_train, hp_n_expert, log_level, GGML_TYPE_F32);
}
...
common_device_memory_data_vec common_get_device_memory_data(
    ...
    std::vector<llama_device_memory_data> impl = common_get_device_memory_data_impl(
            path_model, mparams, cparams, devs, hp_ngl, hp_n_ctx_train, hp_n_expert, log_level, type_s);
...
static void common_params_fit_impl(
        ...
        size_t * margins_s, uint32_t n_ctx_min, enum ggml_log_level log_level, ggml_type type_s) {
    ...
    const dmds_t dmds_full = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, log_level, type_s);
        ...
        if (global_surplus < 0) {
            ...
            if (cparams->n_ctx == 0) {
                if (hp_nct > n_ctx_min) {
                    ...
                    const dmds_t dmds_min_ctx = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, log_level, type_s);
                    if (nd == 0) {
    ...
    auto get_memory_for_layers = [&](
        ...
        const dmds_t dmd_nl = common_get_device_memory_data_impl(
            path_model, &mparams_copy, cparams, devs, hp_ngl, hp_nct, hp_nex, log_level, type_s);
    ...
    if (hp_nex > 0) {
        ...
        const dmds_t dmds_cpu_moe = common_get_device_memory_data_impl(
            path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, log_level, type_s);
...
enum common_params_fit_status common_fit_params(
    ...
    return common_fit_params(
            path_model, mparams, cparams, tensor_split, tensor_buft_overrides, margins, n_ctx_min, log_level, GGML_TYPE_F32);
}
...
enum common_params_fit_status common_fit_params(
    ...
    try {
        common_params_fit_impl(path_model, mparams, cparams, tensor_split, tensor_buft_overrides, margins, n_ctx_min, log_level, type_s);
...
void common_fit_print(
        ...
        llama_model_params * mparams,
        llama_context_params * cparams) {
    common_fit_print(path_model, mparams, cparams, GGML_TYPE_F32);
}
...
void common_fit_print(
    ...
    auto dmd = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, GGML_LOG_LEVEL_ERROR, type_s);
