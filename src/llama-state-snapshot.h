...
struct llama_state_seq_snapshot : llama_io_write_i {
    ...
    explicit llama_state_seq_snapshot(size_t max_bytes) : limit(max_bytes) {
        if (charged > limit) throw std::runtime_error("snapshot budget exceeded");
    }
    void charge(size_t size) {
        if (size > limit - charged) throw std::runtime_error("snapshot budget exceeded");
        charged += size;
    }
    void append(part value) {
        if (frozen || value.size > std::numeric_limits<size_t>::max() - bytes)
            throw std::runtime_error("invalid snapshot write");
    ...
    void write_tensor(ggml_tensor * tensor, size_t offset, size_t size) override {
        ...
        if (!tensor || !tensor->buffer || offset > ggml_nbytes(tensor) || size > ggml_nbytes(tensor) - offset ||
            ...
            throw std::runtime_error("invalid snapshot tensor range");
        ...
        if (!cpu && !cuda)
            throw std::runtime_error("snapshot backend is not supported");
    ...
    void freeze() {
        if (frozen) throw std::runtime_error("snapshot already frozen");
