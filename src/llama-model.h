...
struct llama_model {
    ...
    // Hadamard-folded GGUF weights (prism.hadamard) are matched with persistent
    // model tensors containing the activation-side transform. The string maps
    // are filled from GGUF metadata while loading hparams; the pointer maps
    // after the model buffers exist. Explicit sign mode also brings per-width
