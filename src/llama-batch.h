...
public:
    ...
    // seq_filter selects whole sequence sets; single_seq emits only the first selected set
    llama_ubatch split_equal(uint32_t n_ubatch, bool sequential, uint32_t n_keep_tail,
            const std::function<bool(llama_seq_id)> & seq_filter = {}, bool single_seq = false);
