...
ggml_tensor * llama_model_base::create_tensor(llama_model_loader & ml, const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) {
    if (tn.bid != -1 && tn.bid >= (int) pimpl->dev_layer.size()) {
        throw std::runtime_error(format("create_tensor: %s has layer id %d out of range (dev_layer size %zu)",
                                        tn.str().c_str(), tn.bid, pimpl->dev_layer.size()));
    }
