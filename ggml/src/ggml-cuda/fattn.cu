...
static best_fattn_kernel ggml_cuda_get_best_fattn_kernel(const int device, const ggml_tensor * dst) {
    ...
    if (K_hot != nullptr && ggml_flash_attn_ext_hybrid_mode(dst) == 1) {
        ...
        if (mask != nullptr && (mask->type != GGML_TYPE_I32 || !ggml_is_contiguous(mask) ||
                mask->ne[0] < 1 || (mask->ne[0] - 1) % 3 != 0 ||
                mask->ne[1] != K->ne[2] || mask->ne[2] != k_pos->ne[3])) {
            return BEST_FATTN_KERNEL_NONE;
        }
    ...
    if (ggml_cuda_fattn_q4_batch_invariant(KQV)) {
        GGML_ASSERT(can_use_vector_kernel);
        return BEST_FATTN_KERNEL_VEC;
    }
