...
'@
    ...
    & (Join-Path $scripts 'Invoke-DsparkSpsProfile.ps1') `
        ...
        -OutputDir $output -RuntimeLabel 'cuda-mock' -PythonPath $python `
    ...
    Assert-True ((Split-Path -Parent $capture.StdoutPath) -eq $testRoot) 'guard stdout is not a sibling of OutputDir'
    Assert-True ((Split-Path -Parent $capture.StderrPath) -eq $testRoot) 'guard stderr is not a sibling of OutputDir'
    Assert-True ((Split-Path -Leaf $capture.StdoutPath) -match '^output\.guard-\d{8}T\d{13}Z-\d+-[0-9a-f]{32}\.stdout\.log$') 'guard stdout has no unique run suffix'
    Assert-True ((Split-Path -Leaf $capture.StderrPath) -match '^output\.guard-\d{8}T\d{13}Z-\d+-[0-9a-f]{32}\.stderr\.log$') 'guard stderr has no unique run suffix'
    Assert-True ($capture.StdoutPath -ne $capture.StderrPath) 'guard stdout and stderr paths collide'
    Assert-True ((Get-ChildItem -LiteralPath $output -Force).Count -eq 0) 'fresh OutputDir was polluted before the runner'
    ...
    Assert-True ($runner -contains '--no-dynamic-rs') 'default Dynamic-RS off state was not forwarded explicitly'
    Assert-True (-not ($runner -contains '--dynamic-rs')) 'Dynamic-RS was enabled by default'
    Assert-True ($runner -contains '--no-resume') 'resume switch was not forwarded'

    $dynamicOutput = Join-Path $testRoot 'output-dynamic-rs'
    & (Join-Path $scripts 'Invoke-DsparkSpsProfile.ps1') `
        -PlanPath $plan -ServerPath $server -ModelPath $model -DraftModelPath $draft `
        -OutputDir $dynamicOutput -RuntimeLabel 'cuda-mock-dynamic-rs' -PythonPath $python `
        -DynamicRs $true
    $dynamicCapture = Get-Content -Raw -LiteralPath $capturePath | ConvertFrom-Json
    $dynamicRunner = @($dynamicCapture.TaskArguments)
    Assert-True ($dynamicRunner -contains '--dynamic-rs') 'explicit Dynamic-RS opt-in was not forwarded'
    Assert-True (-not ($dynamicRunner -contains '--no-dynamic-rs')) 'Dynamic-RS opt-in also forwarded the off state'
