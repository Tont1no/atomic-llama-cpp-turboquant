...
int main() {
    // Dynamic recurrent snapshots are an execution-layout property of the
    // pure packed DSpark path, independent of whether its current cap comes
    // from fixed drafting, SPS planning, shadow planning, or the recorder.
    // DFlash retains its adaptive-only opt-in and mixed chains fail closed.
    {
