...
try {
    ...
    $short = & $guardPath -Executable $pwshPath -TaskArguments @(
        ...
        "[IO.File]::WriteAllText('$($visibleGpuPath.Replace("'", "''"))',`$env:CUDA_VISIBLE_DEVICES); Start-Sleep -Milliseconds 50"
...
"@
    ...
    Assert-True $limitFailed 'over-limit task did not fail through the memory guard'
    ...
    try {
        ...
        $descendantFailed = $_.Exception.Message -like '*left relevant processes running*'
    }
